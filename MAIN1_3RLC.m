...
sys_G_ang= CHEN1_3(Amplitud, t_d, V_c_d, t_inic,retardo, 12);

[ychen ,tchen,ent]=lsim(sys_G_ang, Ve_d,t_d, [0,0]);

figure 1
plot(t_d,V_c_d); title('Tension del capacitor');grid on;hold on;
plot(tchen,ychen,'--');
...
plot(t,I,'--');legend('corriente real','corriente aproximada');
subplot(3,1,2);plot(t_d,V_c_d); title('Tension del capacitor');grid on;hold on;
plot(t,V_c,'--');legend('tension real','tensión aproximada');
